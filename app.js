...
const streetLayer = L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}", { attribution: "Tiles © Esri" });
const satelliteLayer = L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", { attribution: "Imagery © Esri" });
const terrainLayer = L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}", { attribution: "Tiles © Esri" });
streetLayer.addTo(map);
L.control.zoom({ position: "bottomright" }).addTo(map);
L.control.layers({ Street: streetLayer, Satellite: satelliteLayer, Terrain: terrainLayer }, null, { position: "bottomright", collapsed: false }).addTo(map);
...
function getFlagImage(country, state) {
    ...
    const brazilFlags = {
        goias: "https://upload.wikimedia.org/wikipedia/commons/b/be/Bandeira_de_Goi%C3%A1s.svg",
    ...
    if (
        ...
        return `<img class="flag-img"
            src="${usFlags[stateKey]}"
            alt="${cleanState} flag">`;
    }

    if (
        cleanCountry === "Brazil" &&
        brazilFlags[stateKey]
    ) {
        return `<img class="flag-img"
            src="${brazilFlags[stateKey]}"
            alt="${cleanState} flag">`;
    }
    ...
    if (code) {
        return `<img class="flag-img"
            ...
            alt="${cleanCountry} flag">`;
    }

    return `<div class="flag-placeholder">🌍</div>`;
}

function missionaryPopupHtml(m){const lang=getLanguage(m),pres=getPresidents(m),spouse=getSpouse(m);return `<div class="leaflet-mission-popup single-missionary-popup"><h2>${getTitle(getSex(m),getName(m),lang)}</h2><div class="card-divider"></div><div class="card-location-small">${getFlagImage(getCountry(m),getState(m))}<div><div class="card-mission">${getMissionName(m)||""}</div>${lang?`<div class="popup-language">${lang}</div>`:""}</div></div><div class="card-dates">${formatMissionDates(m)}</div>${pres?`<div class="card-presidents">${formatPresidents(pres)}</div>`:""}${spouse?`<div class="card-spouse">Married to ${spouse}</div>`:""}</div>`;}
function groupPopupHtml(group){const f=group[0];const langs=new Set(group.map(m=>getLanguage(m)).filter(Boolean));return `<div class="leaflet-mission-popup group-mission-popup"><h2>${getMissionName(f)}</h2><div class="card-divider"></div><div class="card-location-small">${getFlagImage(getCountry(f),getState(f))}<div><div class="card-mission">${group.length} missionaries served here</div>${langs.size?`<div class="popup-language">${Array.from(langs).join(", ")}</div>`:""}</div></div><div class="missionary-list">${group.map((m,i)=>`<button type="button" class="missionary-list-item" data-index="${i}"><strong>${getTitle(getSex(m),getName(m),getLanguage(m))}</strong><span>${formatMissionDate(getStartDate(m))} – ${isCurrentlyServing(m)?"Present":formatMissionDate(getEndDate(m))}</span></button>`).join("")}</div></div>`;}
...
function handleOutsidePreviewClick(e){const p=document.getElementById("detailPreview");if(!p||p.classList.contains("hidden"))return;if(!p.contains(e.target)&&!e.target.closest(".missionary-list-item"))hideDetailPreview();}
function setupGroupPopupEvents(marker,group){marker.on("popupopen",ev=>{setTimeout(()=>{const el=ev.popup.getElement();if(!el)return;el.querySelectorAll(".missionary-list-item").forEach(btn=>{const m=group[Number(btn.dataset.index)];const show=()=>{document.querySelectorAll(".missionary-list-item.active").forEach(b=>b.classList.remove("active"));btn.classList.add("active");showDetailPreview(m,btn);};btn.addEventListener("mouseenter",show);btn.addEventListener("click",e=>{e.preventDefault();e.stopPropagation();show();});});document.addEventListener("click",handleOutsidePreviewClick);},0);});marker.on("popupclose",()=>{hideDetailPreview();document.removeEventListener("click",handleOutsidePreviewClick);});}
function bindMarkerPopup(marker,group){if(group.length===1){marker.bindPopup(missionaryPopupHtml(group[0]),{closeButton:true,autoPan:true,maxWidth:360,className:"haag-leaflet-popup"});}else{marker.bindPopup(groupPopupHtml(group),{closeButton:true,autoPan:true,maxWidth:380,className:"haag-leaflet-popup"});setupGroupPopupEvents(marker,group);}marker.on("mouseover",()=>marker.openPopup());marker.on("click",()=>marker.openPopup());}
function createMarkerIcon(group){const count=group.length,hasMale=group.some(m=>getSex(m)!=="Female"),hasFemale=group.some(m=>getSex(m)==="Female"),hasCurrent=group.some(isCurrentlyServing),hasInLaw=group.some(isInLaw);let color=getColor(getSex(group[0]));if(hasMale&&hasFemale)color="#7c3aed";let inner="";if(count>1)inner=`<span class="marker-number">${count}</span>`;else if(hasCurrent)inner=`<span class="marker-check">✓</span>`;const cls=["mission-marker",count>1?"duplicate-marker":"",hasInLaw&&count===1?"heart-marker":"dot-marker"].join(" ");return L.divIcon({className:"custom-marker",html:`<div class="${cls}" style="--marker-color:${color}">${inner}</div>`,iconSize:[34,34],iconAnchor:[17,17],popupAnchor:[0,-14]});}
...
function matchesSearch(m){if(!activeSearch)return true;const h=[getName(m),getMissionName(m),getCity(m),getState(m),getCountry(m),getLanguage(m),getPresidents(m),getSpouse(m),getRelation(m)].join(" ");return normalize(h).includes(normalize(activeSearch));}
...
function drawMarkers(){clearMarkers();hideDetailPreview();const data=getFilteredData();updateStats(data);const groups=groupByMission(data);const bounds=[];groups.forEach(g=>{const c=g[0].coords;const marker=L.marker([c.lat,c.lng],{icon:createMarkerIcon(g)}).addTo(map);bindMarkerPopup(marker,g);allMarkers.push(marker);bounds.push([c.lat,c.lng]);});if(bounds.length>0)map.fitBounds(bounds,{padding:[80,80],maxZoom:5});}
